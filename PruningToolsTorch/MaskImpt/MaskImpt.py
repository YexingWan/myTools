...
import sys
sys.path.insert(0,'..')
from core.MaskPrune import Mask, _check_make
import logging
from copy import deepcopy
from collections import defaultdict

__all__=['MaskFPGM','MaskFilterNorm','MaskThiNet','MaskAPoZ']
...
class MaskFPGM(Mask):
    ...
    def __update_mask(self,para_name:str,module_name:str,new_mask:np.ndarray,filter_index:np.ndarray) -> set:
        ...
        # bn, next_conv/fc
        for node in self.model_graph.nodes():
            if "convolution" in node.kind() and self.scope2name(node.scopeName()) == module_name:
                related_nodes,_ = self.general_search(node)
                logging.debug("pruning filter of {} => pruning channel of {}".format(module_name,[self.scope2name(n.scopeName()) for n in related_nodes]))
                assert (len(related_nodes) > 0)
                for r_node in related_nodes:
                    # bn
                    if "batch_norm" in r_node.kind():
                        for sub_para_name in self.moduels_para_map[self.scope2name(r_node.scopeName())]:
    ...
    def __update_mask(self,para_name:str,module_name:str,new_mask:np.ndarray,filter_index:np.ndarray) -> set:
        ...
        for node in self.model_graph.nodes():
            if "convolution" in node.kind() and self.scope2name(node.scopeName()) == module_name:
                ...
                logging.debug("pruning channel of {} => pruning filter of {}".format(module_name,[self.scope2name(n.scopeName()) for n in related_nodes]))
                assert (len(related_nodes) > 0)
                for r_node in related_nodes:
                    # bn
                    if "batch_norm" in r_node.kind():
...
class MaskAPoZ(Mask):
    ...
    def custom_init(self):
            ...
            if "convolution" in node.kind():
                ...
                for relu_node in result_list:
                    if "relu_" in relu_node.kind():
                        # self.relu_conv_name_map[self.scope2name(relu_node.scopeName())].append(self.scope2name(
                        #     node.scopeName()))
                        self.conv_relu_name_map[self.scope2name(node.scopeName())] = \
                            self.scope2name(relu_node.scopeName())
                        relu_set.add(self.scope2name(relu_node.scopeName()))
                logging.debug("put parameter name of module {} in self.mask_name".format(module_name + '.weight'))
                self.mask_name.append(module_name + '.weight')
        logging.debug("Current mask_name is {}".format(self.mask_name))
        ...
        for node in self.model_graph.nodes():
            name = self.scope2name(node.scopeName())
            if name in relu_set:
                # input node of relu
                ...
                if not all(valid_list):
                    self.relu_module_name_map[name].append(self.scope2name(before_node.scopeName()))
                # else, find last conv before
                    ...
                    self.relu_module_name_map[name].append(self.scope2name(conv_list[0].scopeName()))
        ...
        #             self.relu_conv_name_map[self.scope2name(conv_list[0].scopeName())] = self.scope2name(node.scopeName())
        #             self.conv_relu_name_map[self.scope2name(node.scopeName())].append(self.scope2name(conv_list[0].scopeName()))
        #             self.relu_list.append(self.scope2name(node.scopeName()))
        #
        #         else:
        #             logging.debug("skip relu module {}".format(self.scope2name(node.scopeName())))
